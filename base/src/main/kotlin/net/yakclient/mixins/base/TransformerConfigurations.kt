            ...
            for (clazz in from) {
                for (method in clazz.methods) {
                    if (method.isAnnotationPresent(Injection::class.java)) {
                        ...
                        val signature =  injection.to.takeIf { it == METHOD_SELF } ?: ByteCodeUtils.byteCodeSignature(method)
                        addMt(TargetedMethodTransformer(MixinInjectionTransformer(Injectors.of(type), type, source), signature))
                    }
                }
            }
        }
