...
public class ClassTarget extends PackageTarget {
    ...
    public static ClassTarget of(String path) {
        try {
            if (ByteCodeUtils.classExists(path)) {
                final String[] fromPath = fromPath(path);
    ...
    public static ClassTarget of(Class<?> path) {
        final String[] fromPath = fromPath(path.getName());
